...
var onboardCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if onboardApiKey == "" {
			onboardApiKey = config.ProviderAPIKey(onboardProvider)
		}

		// Translating flags back into slice to reuse current HandleOnboard cleanly
		var passArgs []string
		if onboardProvider != "" {
			passArgs = append(passArgs, "--provider", onboardProvider)
		}
		if onboardModel != "" {
			passArgs = append(passArgs, "--model", onboardModel)
		}
		if onboardApiKey != "" {
			passArgs = append(passArgs, "--api-key", onboardApiKey)
		}
...
func init() {
	...
	onboardCmd.Flags().StringVar(&onboardApiKey, "api-key", "", "Provider API Key (defaults to DEEPSEEK_API_KEY/PROVIDER_API_KEY env)")
